export function parseIPv4AddressString(addressString: string): Buffer {
  ...
  const parts = stringParts.map((p) => {
    if (!/^\d+$/.test(p)) {
      throw new Error(
        `Invalid IPv4 address ${addressString}: part "${p}" contains non-digit characters.`
      );
    }
    const parsed = parseInt(p, 10);
    if (parsed < 0 || parsed > 255) {
      throw new Error(
        `Invalid IPv4 address ${addressString}: part "${p}" is not a valid octet.`
      );
    }
    return parsed;
  });
...
export function parseIPv6AddressString(addressString: string): Buffer {
  ...
  if (addressString.includes("::")) {
    ...
    if (partsSplitted.length > 2) {
      throw new Error(
        `Invalid IPv6 address ${addressString}: multiple '::' sequences.`
      );
    }
    ...
    if (zeroFillCount < 0) {
      throw new Error(
        `Invalid IPv6 address ${addressString}: too many parts for '::' expansion.`
      );
    }
  ...
  if (parts.length !== 8) {
    throw new Error(
      `Invalid IPv6 address ${addressString}: expected 8 parts, got ${parts.length}.`
    );
  }
  ...
  for (let i = 0; i < 8; i++) {
    ...
    if (part.length > 4) {
      throw new Error(
        `Invalid IPv6 address ${addressString}: part length: ${part}`
      );
    }
...
export function formatIPv6AddressBuffer(addressBuffer: Buffer): string {
  ...
  if (addressBuffer.length !== 16) {
    throw new Error(
      `Invalid buffer length for IPv6 address ${addressBuffer}: expected 16 bytes.`
    );
  }
...
export function formatIPAddressBuffer(addressBuffer: Buffer): string {
  ...
  throw new Error(
    `Invalid buffer length for IP address ${addressBuffer}: expected 4 or 16 bytes.`
  );
}
