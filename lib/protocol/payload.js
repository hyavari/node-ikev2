...
class PayloadSA extends Payload {
    ...
    static parse(buffer) {
        ...
        while (offset < genericPayload.length) {
            const proposal = proposal_1.Proposal.parse(buffer.subarray(offset));
...
class PayloadKE extends Payload {
    ...
    static parse(buffer) {
        ...
        const keyData = buffer.subarray(8);
...
class PayloadID extends Payload {
    ...
    static parse(buffer) {
        ...
        const idData = buffer.subarray(8);
...
class PayloadCERT extends Payload {
    ...
    static parse(buffer) {
        ...
        const certData = buffer.subarray(8);
...
class PayloadCERTREQ extends Payload {
    ...
    static parse(buffer) {
        const genericPayload = Payload.parse(buffer);
        const certEncoding = buffer.readUInt8(4);
        const certAuthority = buffer.subarray(8);
...
class PayloadAUTH extends Payload {
    ...
    static parse(buffer) {
        ...
        const authData = buffer.subarray(8);
...
class PayloadNONCE extends Payload {
    ...
    static parse(buffer) {
        const genericPayload = Payload.parse(buffer);
        const nonceData = buffer.subarray(4);
...
class PayloadNOTIFY extends Payload {
    ...
    static parse(buffer) {
        ...
        const notifyData = buffer.subarray(8 + spiSize);
...
class PayloadVENDOR extends Payload {
    ...
    static parse(buffer) {
        const genericPayload = Payload.parse(buffer);
        const vendorId = buffer.subarray(4);
...
class PayloadTS extends Payload {
    ...
    static parse(buffer) {
        ...
        for (let i = 0; i < numTs; i++) {
            const ts = selector_1.TrafficSelector.parse(buffer.subarray(offset));
            tsList.push(ts);
            offset += ts.length;
        }
...
class PayloadSK extends Payload {
    ...
    static parse(buffer) {
        const genericPayload = Payload.parse(buffer);
        const encryptedData = buffer.subarray(4);
...
class PayloadCP extends Payload {
    ...
    static parse(buffer) {
        ...
        const cfgData = buffer.subarray(8);
...
class PayloadEAP extends Payload {
    ...
    static parse(buffer) {
        const genericPayload = Payload.parse(buffer);
        const tlvData = attribute_1.Attribute.parse(buffer.subarray(4));
